...
int UART0_Open(char* port)
{
    ...
    {
       printf("Redsky UART:fcntl failed!\n");
       return(FALSE);
    }
    else
    {
       printf("Redsky UART:fcntl=%d\n",fcntl(fd, F_SETFL,0));
    }
    //测试是否为终端设备
    /*
    if(0 == isatty(STDIN_FILENO))
    {
         printf("Redsky UART:standard input is not a terminal device\n");
         return(FALSE);
    }
    else
    {
        printf("Redsky UART:isatty success!\n");
    }
    */
    printf("Redsky UART:fd->open=%d\n",fd);
    return fd;
}
/*******************************************************************
* 名称：                UART0_Set
...
int UART0_Set(int fd,int speed,int flow_ctrl,int databits,int stopbits,int parity)
{
    unsigned int   i;
      //int   status;
    int   speed_arr[] = { B115200,B38400, B19200, B9600, B4800, B2400, B1200, B300,B115200,
